...
if [ -e /etc/openvpn/server.conf ]; then
	...
	do
	clear
		echo "Lihatlah, OpenVPN telah terpasang di sistem kamu"
		echo "Apa yang akan kamu lakukan untuk hal ini?"
		echo ""
		echo "1) Hapus OpenVPN dari sistem"
		echo "2) Keluar"
		echo ""
		...
		case $option in
			...
			sed -i '/iptables -t nat -A POSTROUTING -s 192.168.100.0/d' /etc/rc.local
			echo ""
			echo "OpenVPN selesai dihapus !"
			exit
			;;
			2) exit;;
		esac
	...
	echo 'Selamat Datang di OpenVPN Installer ala VPS Workshop'
	echo "Modifikasi Oleh Satria Aji Putra VII"
	echo ""
	...
	echo ""
	echo "Apakah Kamu ingin OpenVPN akan tersedia pada port 53 juga?"
	...
	echo ""
	echo "Oke, itu semua saya butuhkan. Kami siap untuk setup OpenVPN server kamu sekarang"
